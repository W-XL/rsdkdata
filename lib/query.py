...
class Query(object):
    def __init__(self, table_name = None, db = None, mmc = None):
        self.write = 0
    ...
    def _re_connect_db(self,db):
        ...
        #             user=options.mysql_user_write, password=options.mysql_password_write
        #         )
        self.db.close()
        if self.write == 1:
            self.db = torndb.Connection(
    ...
    def add(self, cheat = False):
        ...
        sql = self.__sqlfix(sql)
        self.__close()
        return self.db.execute_lastrowid(sql) if not cheat else sql

    def insert(self,sql):
        return self.db.execute_lastrowid(sql)

    def pages(self, current_page = 1, list_rows = 40, cheat = False ,manual = False):
        sql = self.select(cheat = True)
        self.__close()
        if manual == False:
            count = self.grasp(sql).count()
        else:
            count = self.m_count(sql)
        ...
        if manual== False:
            ...
            result["list"] = self.query(sql+" limit "+str(start)+","+str(end))
    ...
    def m_count(self, sql):
        self._re_connect_db(self.db)
        sql = "SELECT COUNT(*) AS COUNT FROM (%s)s" % sql
        sql = self.__sqlfix(sql)
        self.__close()
        data = self.db.query(sql)
        return data[0]['COUNT']

    def switch_to_write_db(self, db_write):
        self.write = 1
        self.db = db_write

    def switch_to_read_db(self, db_read):
        self.write = 0
        self.db = db_read
